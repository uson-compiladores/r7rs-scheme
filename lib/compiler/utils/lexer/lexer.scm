(define (rule-make token regex callback)
...
(define rules
  (list
   (rule-make 'UNO (rx (.. (* (c "a")) (c "b"))) (lambda (lexeme) 1))
   (rule-make 'DOS (rx (.. (* (c "a")) (c "c"))) (lambda (lexeme) 2))))
...
(define (rule-map rules char)
  (if (null? rules)
	...
	(if (null? d)
	    ...
	    (cons (rule-make (rule-token (car rules))
			     d
