(library (streams primitive) (export stream-null stream-cons stream? stream-null? stream-pair? stream-car stream-cdr stream-lambda) (define-record-type (stream-type make-stream stream?) (fields (mutable box stream-promise stream-promise!)))
  
  ;; (stream-lazy expr) is like (box (cons 'lazy (lambda () expr)))
  (define-syntax stream-lazy		; equivalent to (delay expr)
    (syntax-rules ()
      ((stream-lazy expr)
        (make-stream
          (cons 'lazy (lambda () expr))))))

  ;; (stream-eager expr) is like (box (cons 'eager expr))
  (define (stream-eager expr)		; equivalent to (make-promise expr)
    (make-stream
      (cons 'eager expr)))

  ;; (stream-delay expr) is like (box (cons 'lazy (lambda () (box (cons 'eager expr)))))
  (define-syntax stream-delay		; equivalent to (delay (make-promise expr))
    (syntax-rules ()
      ((stream-delay expr)
    ...
    (let ((content (stream-promise promise)))
      (case (car content)
        ...
        ((lazy)  (let* ((promise* ((cdr content)))
			;; if stream with stream-lazy then promise* is expr
			;; if stream with stream-delay then promise* is (box (cons 'eager expr))
                        (content  (stream-promise promise))
			;; if stream with stream-lazy (cons 'lazy (lambda () expr))
			;; if stream with stream-delay then content is (cons 'eager expr)
			)
